...
import { Avatar, Button, Dropdown, Heading, Menus, Table, notification } from '@/components/ui'
...
const Members = observer(() => {
  ...
  const getId = localStorage.getItem('UserStore') ?? '{}';
  const userId = JSON.parse(getId);
  const [toggle, settoggle] = useState(true)
  console.log("🚀 ~ Members ~ toggle:", toggle)
  ...
  //buyplan
  const [buyPlanVisible, openBuyPlan, closeBuyPlan] = useVisible()
    ...
    <div>
      <Heading
        ...
        actions={
          <>
            {/* buyplan */}
            <Button type="primary" disabled={!toggle} onClick={openBuyPlan}>
              {t('workspace.members.index.buyPlan')}
            </Button>
      ...
      {/* Buy plan */}
      <Buyplan visible={buyPlanVisible} onClose={closeBuyPlan} />
